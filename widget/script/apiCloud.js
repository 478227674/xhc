...
function getImgSizeBySize(url,showWidth,showHieght){
    if(isBlack(url) &&  !url.startWith("http")){
        return {w:showWidth,h:showHieght};
    }
...
function getCode() {
    ...
    flag = false;
    var tel = $api.byId('sjphone').value;
    if (!checkMobileNum(tel)) {
        toast("手机号码不正确");
        return;
    }
    var vCode = $("#imgCodeText").val();
    if(vCode){
    ...
    ajaxGetVistor(getCodeUrl, {phone: tel,verifyCode:vCode}, function (ret,err) {
        if (ret && ret.success) {
            $(".zz").hide();
            toast('获取成功');
            setTime();
...
function pushImgCode() {
    phone = $("#sjphone").val();
    var fl = $api.getStorage(Storage_Sms_Time);
    var newtime =  Date.parse(new Date());
    if(phone){
        if(fl){
            if(newtime-fl>120*1000){
                $(".zz").show();
...
function changeImgCode() {
    var url = getVerifyCodeUrl + "?phone=" + phone + "&time" + Date.parse(new Date());
    $("#imgCode").attr('src',url)
}
...
function setTime(time) {
    ...
    var codeInterval = setInterval(function () {
        if (time > 0) {
            ...
            sms.style.backgroundColor = "#b22a2a";
            sms.style.color = "#fff";
            sms.innerHTML = "获取";
...
function base64ToFiel(base64Url,name) {
...
function openFrameHeight(name,url,pageParam,headHeight,bottomHeight,overload,height) {
    var params = {
        name : name,
        url : url,
        pageParam : pageParam,
